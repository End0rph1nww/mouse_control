...
_MOONDREAM_MODULES: tuple | None = None
_MOONDREAM_IMPORT_ERROR: str | None = None


def _get_moondream_modules() -> tuple | None:
    """延迟导入 Moondream 模块，缓存结果。mss 截图比 pyautogui 清晰（物理分辨率）。"""
    global _MOONDREAM_MODULES, _MOONDREAM_IMPORT_ERROR
    if _MOONDREAM_MODULES is not None:
        return _MOONDREAM_MODULES
    if _MOONDREAM_IMPORT_ERROR is not None:
        return None
    try:
        ...
        _MOONDREAM_MODULES = (
            grab_screen_png, load_config, get_model,
            is_tool_ready, start_preload_model, loading_status_message, runtime,
        )
        return _MOONDREAM_MODULES
    except ImportError as e:
        _MOONDREAM_IMPORT_ERROR = str(e)
        return None
...
def _mouse_visual_locate_impl(desc: str) -> dict[str, Any]:
    mods = _get_moondream_modules()
    if mods is None:
        return {
            "error": (
                "视觉定位需要 Moondream 识屏插件。"
                "请在插件管理器中启用 moondream_vision。"
                + (f" 导入失败: {_MOONDREAM_IMPORT_ERROR}" if _MOONDREAM_IMPORT_ERROR else "")
            )
        }
...
def _visual_locate_best(description: str) -> dict[str, Any]:
    """视觉定位：OmniParser 优先，回退 Moondream detect/point。"""
    # 1. OmniParser
    try:
        omni = _call_omniparser(_get_pg().screenshot())
        if "error" not in omni and omni.get("elements"):
            q = description.strip().lower()
            for e in omni["elements"]:
                if q in (e.get("text", "") or "").lower() or q in (e.get("type", "") or "").lower():
                    return {
                        "method": "omniparser",
                        "description": description,
                        "x_pct": e["x_pct"], "y_pct": e["y_pct"],
                        "x_pixel": int(e["x_pct"] * _SCREEN_W),
                        "y_pixel": int(e["y_pct"] * _SCREEN_H),
                        "screen_size": [_SCREEN_W, _SCREEN_H],
                        "matched_element": e,
                    }
    except Exception:
...
def _moondream_point_coords(desc: str) -> tuple[float, float] | None:
    ...
    try:
        ...
        if mods is None:
            return None
        grab_screen_png, load_config, get_model, is_tool_ready, start_preload_model, loading_status_message, runtime = mods
        import io as _io
